...
import * as constants from 'core/utilities/constants';
import { indexBy } from 'core/utilities/common';
import { useVaiUser } from 'core/hooks/useVaiUser';
import { useComptroller, useVenusLens } from 'core/hooks/useContract';
    ...
    const updateMarketUserInfo = async () => {
      ...
      try {
        ...
        const assetsIn = account
          ? await comptrollerContract['getAssetsIn'](account)
          : [];
        ...
        if (account) {
          balances = indexBy(
            ...
            await lens['vTokenBalancesAll'](vtAddresses, account)
          );
        ...
        let assetList = Object.values(constants.CONTRACT_TOKEN_ADDRESS).map(
          (item, index) => {
            const toDecimalAmount = (mantissa) => {
              return new BigNumber(mantissa.toString()).shiftedBy(
                -item.decimals
              );
            };
            ...
            const vtokenAddress =
              constants.CONTRACT_VBEP_ADDRESS[item.id].address.toLowerCase();
        ...
        assetList = await Promise.all(
          assetList.map(async (asset) => {
            const getHypotheticalLiquidity = () => {
              return comptrollerContract.getHypotheticalAccountLiquidity(
                account,
                asset.vtokenAddress,
        ...
        const totalSupplied = assetList.reduce((acc, asset) => {
          const supplyBalanceUSD = asset.supplyBalance.times(asset.tokenPrice);
          return acc.plus(supplyBalanceUSD);
        }, new BigNumber(0));
        ...
        setUserTotalSupplyBalance(totalSupplied);
